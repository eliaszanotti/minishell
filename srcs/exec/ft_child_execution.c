...
/*   Updated: 2023/06/22 11:04:09 by tgiraudo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	ft_check_path(t_args *args, char *path, char **command)
{
	if (path[ft_strlen(path) - 1] == '/')
	{
		...
		{
			free(path);
			ft_exit_and_free(args, 1264, command[0], command);
		}
		else
		{
			free(path);
			ft_exit_and_free(args, 1265, command[0], command);
		}
	}
	if (ft_check_stat(path))
	{
...
static int	ft_dup_and_exec(t_args *args, char **command, int last, int fd[2])
{
	...
	if (!path)
		ft_exit_and_free(args, 1270, command[0], command);
	if (ft_check_path(args, path, command))
		return (1);
	char_envp = ft_get_char_envp(args);
	execve(path, command, char_envp);
	ft_free_str(char_envp);
	free(path);
	ft_exit_and_free(args, 1262, NULL, command);
	return (0);
}
